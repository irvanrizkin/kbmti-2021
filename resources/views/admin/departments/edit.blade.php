...
<div class="card">
    ...
    <div class="card-body">
        ...
        <form method="POST" action="{{ route("admin.departments.update", [$department->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label class="required" for="initial">{{ trans('cruds.department.fields.initial') }}</label>
                <input class="form-control {{ $errors->has('initial') ? 'is-invalid' : '' }}" type="text" name="initial" id="initial" value="{{ old('initial', $department->initial) }}" required>
                @if($errors->has('initial'))
                    <span class="text-danger">{{ $errors->first('initial') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.department.fields.initial_helper') }}</span>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label class="required">{{ trans('cruds.department.fields.type') }}</label>
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type" required>
                    <option value disabled {{ old('type', null) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                    @foreach(App\Models\Department::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $department->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
                    <span class="text-danger">{{ $errors->first('type') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.department.fields.type_helper') }}</span>
            </div>
            <div class="form-group">
                <label class="required">{{ trans('cruds.department.fields.sub_type') }}</label>
                <select class="form-control {{ $errors->has('sub_type') ? 'is-invalid' : '' }}" name="sub_type" id="sub_type" required>
                    ...
                    @foreach(App\Models\Department::SUB_TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('sub_type', $department->sub_type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('sub_type'))
                    <span class="text-danger">{{ $errors->first('sub_type') }}</span>
                @endif
                <span class="help-block">{{ trans('cruds.department.fields.sub_type_helper') }}</span>
            </div>
            <div class="form-group">
