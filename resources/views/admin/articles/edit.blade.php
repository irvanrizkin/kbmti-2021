    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.articles.update", [$article->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
              ...
              <select class="form-control {{ $errors->has('bureau') ? 'is-invalid' : '' }}" name="bureau" id="bureau" required>
                  ...
                  @foreach(App\Models\Article::BUREAU_SELECT as $key => $label)
                      <option value="{{ $key }}" {{ old('bureau', $article->bureau) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                  @endforeach
              </select>
            ...
            <div class="form-group">
              ...
              <input class="form-control date {{ $errors->has('date_upload') ? 'is-invalid' : '' }}" type="text" name="date_upload" id="date_upload" value="{{ old('date_upload', $article->date_upload) }}" required>
              ...
              <label for="tags">{{ trans('cruds.article.fields.tags') }}</label>
                ...
                <select class="form-control select2 {{ $errors->has('tags') ? 'is-invalid' : '' }}" name="tags[]" id="tags" multiple>
                    @foreach($tags as $id => $tag)
                        <option value="{{ $tag->id }}" {{ in_array($id, old('tags', [])) || in_array($tag->id, $arrayedTagId)  ? 'selected' : '' }}>{{ $tag->name }}</option>
                    @endforeach
                </select>
                @if($errors->has('tags'))
