...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.pemilwaCandidate.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.pemilwa-candidates.update", [$pemilwaCandidate->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.pemilwaCandidate.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $pemilwaCandidate->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('no_urut') ? 'is-invalid' : '' }}" type="text" name="no_urut" id="no_urut" value="{{ old('no_urut', $pemilwaCandidate->no_urut) }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('type') ? 'is-invalid' : '' }}" name="type" id="type">
                    ...
                    @foreach(App\Models\PemilwaCandidate::TYPE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('type', $pemilwaCandidate->type) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @if($errors->has('type'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('pemilwa_event') ? 'is-invalid' : '' }}" name="pemilwa_event_id" id="pemilwa_event_id">
                    @foreach($pemilwa_events as $id => $entry)
                        <option value="{{ $id }}" {{ (old('pemilwa_event_id') ? old('pemilwa_event_id') : $pemilwaCandidate->pemilwa_event->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
...
<script>
    ...
    init: function () {
@if(isset($pemilwaCandidate) && $pemilwaCandidate->getMediaPath())
      var file = {!! json_encode($pemilwaCandidate->getMediaPath()) !!}
