...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.upcomingProker.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.upcoming-prokers.update", [$upcomingProker->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label class="required" for="name">{{ trans('cruds.upcomingProker.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $upcomingProker->name) }}" required>
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('caption') ? 'is-invalid' : '' }}" type="text" name="caption" id="caption" value="{{ old('caption', $upcomingProker->caption) }}" required>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" type="text" name="description" id="description" value="{{ old('description', $upcomingProker->description) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control {{ $errors->has('is_active') ? 'is-invalid' : '' }}" name="is_active" id="is_active">
                    ...
                    @foreach(App\Models\UpcomingProker::IS_ACTIVE_SELECT as $key => $label)
                        <option value="{{ $key }}" {{ old('is_active', $upcomingProker->is_active) === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                    @endforeach
                </select>
...
<script>
    ...
    init: function () {
@if(isset($upcomingProker) && $upcomingProker->getMediaPath())
      var file = {!! json_encode($upcomingProker->getMediaPath()) !!}
