...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.eventFieldChoice.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.event-field-choices.store") }}" enctype="multipart/form-data">
            @csrf
            <div class="form-group">
                <label for="choice">{{ trans('cruds.eventFieldChoice.fields.choice') }}</label>
                <input class="form-control {{ $errors->has('choice') ? 'is-invalid' : '' }}" type="text" name="choice" id="choice" value="{{ old('choice', '') }}">
